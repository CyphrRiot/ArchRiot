...
func main() {
	...
	if len(os.Args) > 1 {
		...
		case "--kanshi-autogen":
			// Generate kanshi config based on current monitors; disable internal eDP when externals are present
			{
				// Read hyprctl -j monitors
				...
				for _, m := range mons {
					...
					if strings.HasPrefix(up, "EDP-") || strings.Contains(up, "EDP") || strings.HasPrefix(up, "LVDS") || strings.HasPrefix(up, "DSI") {
				...
				if len(internals) > 0 {
					...
					// Fallback: no internal detected, enable the first monitor
				...
				fmt.Println("✓ Wrote kanshi config:", cfgPath)
